...
def update_prometheus_config(new_target):
    ...
    try:
        ...
        logging.error(f"Permission denied when trying to read {config_path}")
        raise Exception(f"Cannot read Prometheus config file. Ensure the script has read permissions for {config_path}")
    ...
    try:
        ...
        logging.error(f"Permission denied when trying to write to {config_path}")
        raise Exception(f"Cannot write to Prometheus config file. Ensure the script has write permissions for {config_path}")

    # Reload Prometheus configuration by sending SIGHUP
    try:
        pid = int(subprocess.check_output(["pgrep", "-f", "/opt/prometheus/prometheus"]).decode().strip())
        os.kill(pid, signal.SIGHUP)
        logging.info(f"Sent SIGHUP to Prometheus process (PID: {pid})")
    except subprocess.CalledProcessError:
        logging.error("Failed to find Prometheus process")
        raise Exception("Failed to find Prometheus process. Ensure Prometheus is running.")
    except PermissionError:
        logging.error("Permission denied when trying to send SIGHUP to Prometheus")
        raise Exception("Permission denied when trying to send SIGHUP to Prometheus. Ensure the script has necessary permissions.")
